  ...
  def startup() {
    try {
      ...
      if(startupComplete.get)
        return
      //CAS设置isStartingUp状态，防止多线程同时走到这里
      val canStartup = isStartingUp.compareAndSet(false, true)
      if (canStartup) {
        //设置为Starting状态
        brokerState.newState(Starting)

        //启动kafka定时器，其他类中使用其设置定时任务
        /* start scheduler */
        ...
        //配置限制管理器，限制producer、consumer的拉取、消费速率等
